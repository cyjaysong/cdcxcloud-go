...
func (c *Client) PatternAdd(pattern []string) (resBody *PatternAddResBody, err error) {
	data := map[string]any{"action": "patternAdd", "account": c.Account, "password": c.Password, "pattern": pattern}
...
func (c *Client) PatternSignAdd(pattern []string) (resBody *PatternAddResBody, err error) {
	data := map[string]any{"action": "patternSignAdd", "account": c.Account, "password": c.Password, "pattern": pattern}
	bodyBytes, err := c.post(data)
	if err != nil {
		return nil, err
	}
